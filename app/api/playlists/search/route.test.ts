...
describe('GET /api/playlists/search', () => {
    ...
    it('should return playlists if query parameter is provided', async () => {
        ...
        mock.onGet('http://localhost:8080/api/playlists/search?query=test&offset=0&limit=20').reply(200, mockPlaylists)
        
        const req = new NextRequest('http://localhost:3000/api/playlists/search?query=test&offset=0&limit=20')
        const res = await GET(req)
    ...
    it('should return 500 if there is an error fetching playlists', async () => {
        mock.onGet('http://localhost:8080/api/playlists/search?query=test&offset=0&limit=20').reply(500)
    ...
    it('should log appropriate messages during the request lifecycle', async () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith('クエリパラメータ: test, offset: 0, limit: 20')
